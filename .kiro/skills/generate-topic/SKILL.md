...
2. **Write the lesson** — using synthesized research. Follow teach skill conventions (SVG diagram with CSS vars, citations, key-concept blocks, exercise with hint + answer). The exercise tests the lesson's **Win statement** — core concept comprehension, not detail recall or gotchas. For exercise design and the decision/callout hierarchy (including the required "When to use which" criteria when presenting alternatives), follow `visual-teaching.md` (§ Exercise Design, § Callout Hierarchy) — the owner of those rules. Call `python3 -c "from tools.lib.page_template import render_lesson_page; ..."` or have the agent write the body to a temp file and wrap with the template.
...
4. **Write SR questions** — append to `learning-records/questions/{domain}.jsonl`. 4-8 open-answer questions + 2-3 interactive questions per topic. Follow the archetypes, criteria format, `eli5`, provenance/level tagging, and difficulty ordering in [references/sr-question-authoring.md](./references/sr-question-authoring.md) — the single owner of SR-question rules.
