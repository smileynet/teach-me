---
id: "324"
title: "godot-asset-pipeline topic 2: format-decision-and-import-verify"
status: open
blocked_by: ["305", "323"]
validation_criteria:
  ...
  - "Runnable artifact (prop exported multiple ways) + asset:validate-gd L2 assertion (correct cull_mode round-trip)"
  - "Reference doc + SR cards + glossary JSON; passes mise run verify"
tags: ["content"]
---

# godot-asset-pipeline topic 2: format-decision-and-import-verify

## Context

Track spiral topic 2 (#305). Structure: win → complication → resolution → win. Design:
...
- **Start from the win** (topic 1): "You imported one glTF prop and it worked."
...
- [ ] Lesson `02-format-decision-and-import-verify.html` in win→complication→resolution→win shape; Decision callout with real criteria
- [ ] Runnable artifact (prop exported multiple ways) + `asset:validate-gd` L2 assertion (correct `cull_mode` round-trip)
